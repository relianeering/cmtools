...
# The plot is designed to gnerate in pdf multispectra plots from acceleration data
#
# startF = 0
# endF = 1000
# rpm = 60
...
set label 2 sprintf("Spectrum max: %3.3f g at %4.3f Hz | Overall: %3.3f g rms", A_max, (A_index_max * resolution), rms_A) at graph 0, graph -0.15
set label 3 sprintf("Band (%d to %d Hz) max: %3.3f g at %4.3f Hz | Overall: %3.3f g rms", startF, endF, B_max_y, ((B_index_max_y + startInd -1) * resolution), rms_B) at graph 0, graph -0.2
#
# Plot and grid setup
#
set border 0
...
plot [startF:endF] datafile every ::startInd::endInd using (($0 + startInd - 1)*resolution):2 ls 1 with lines title("ACC")
